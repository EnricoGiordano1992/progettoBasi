...
`\textbf{DB to page-schema PazientePage (}`
	...
	cartelle_cliniche: 
			`\color{blue}{select c.*}`
			`\color{blue}{from cartella_clinica as c, paziente as p}`
			`\color{blue}{where c.codsan = ?codsan?}`
			`\color{blue}{and c.codsan = ?codsan?}`

	fattori_rischio: 
			`\color{blue}{select r.*}`
			`\color{blue}{from paziente as p, rischi_paziente as r}`
			`\color{blue}{where p.codsan = (:codsan)}`
			`\color{blue}{and p.codsan = r.id_paziente}`

	elenco_medici: 	
			`\color{blue}{select distinct m.}`
			`\color{blue}{from paziente as p, medico as m, diagnosi as d}`
			`\color{blue}{where p.codsan = ?codsan)?}`
			`\color{blue}{and p.codsan = d.id_paziente}`
			`\color{blue}{and m.id = d.id_medico}`
`\textbf{);}`
\end{lstlisting}
\newpage
\begin{lstlisting}
...
`\textbf{DB to page-schema CartellaPage (}`
	...
	terapie: 
		`\color{blue}{select t.*}`
		`\color{blue}{from cartella_clinica as c, terapie as t}`
		`\color{blue}{where c.id = ?id?}`
		`\color{blue}{and c.id = t.id_cartella}`

	diagnosi:
		`\color{blue}{select d.*, m.*}`
		`\color{blue}{from cartella_clinica as c, diagnosi as d,}`
		`\color{blue}{paziente as p, medico as m}`
		`\color{blue}{where c.id = ?id?}`
		`\color{blue}{and p.codsan = d.id_paziente}`
		`\color{blue}{and p.codsan = c.codsan}`
		`\color{blue}{and d.id_medico = m.id}`
